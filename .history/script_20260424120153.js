...
function draw() {
    ...
    if (faces.length > 0) {
        ...
        if (hasPrev) {
            // velocity
            let vx = raw_cx - prev_rawCx
            let vy = raw_cy - prev_rawCy

            // predict
            pred_cx = raw_cx + vx * 0.25;
            pred_cy = raw_cy + vy * 0.25;
        }
        ...
        if (isFirstDetect) {
            ...
            cx = lerp(cx, pred_cx, 0.75);
            cy = lerp(cy, pred_cy, 0.75);
            d = lerp(d, raw_d, 0.75);
        }
        ...
        if (isFirstAngle) {
            ...
            angle = lerp(angle, raw_angle, 0.4);
        }
