...
function draw() {
    ...
    if (faces.length > 0) {
        ...
        if (isFirstDetect) {
            ...
            raw_yaw_2 = lerp(yaw, raw_yaw, 0.1);
            yaw = constrain(raw_yaw_2, -0.66)

            pitch = lerp(pitch, raw_pitch, 0.9);
        }

        let offsetX = -yaw * d * 0.25;
        let offsetY = pitch * d * 0.5;
        ...
        let raw_warpX = abs(yaw) * w * 0.12;
        let raw_warpY = abs(yaw) * h * 0.25;

        warpX = lerp(warpX, raw_warpX, 0.3);
        warpY = lerp(warpY, raw_warpY, 0.3);

        let plus_strength = 0.4
        let minus_strength = 0.15

        if (yaw > 0) { //kiri
            // kanan lebih besar
            trX += warpX * plus_strength;
            brX += warpX * plus_strength;
            ...
            tlY += warpY * minus_strength;
            blY -= warpY * minus_strength;

        } else { //kanan
            // kiri lebih besar
            tlX -= warpX * plus_strength;
            blX -= warpX * plus_strength;
            ...
            trY += warpY * minus_strength;
            brY -= warpY * minus_strength;
        }
